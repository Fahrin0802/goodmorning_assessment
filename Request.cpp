...
void Request::simulateLatency(const std::string& uri)
{
    ...
    static std::normal_distribution<double> defaultDistribution(15000.0, 5000.0);
    static std::random_device device;
   
    const auto it = distributions.find(uri);
    std::normal_distribution<double>& distribution = (it == distributions.end() ? defaultDistribution : it->second);

    const auto responseTime = distribution(device);
    if (responseTime >= 1.0)
    {
        std::this_thread::sleep_for(std::chrono::microseconds(static_cast<std::chrono::microseconds::rep>(responseTime)));
    }
}
